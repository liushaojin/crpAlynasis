...
MV200_MainWidget *gMV200_MainWidget = 0;
MV200_MainWidget::MV200_MainWidget(QWidget *parent) : QMainWindow(parent)
{
    initForm();          //初始化
    initStyle();         //界面风格
    initConnect();       //连接信号与槽
...
//初始化
void MV200_MainWidget::initForm()
{
...
//功能菜单工具栏（顶部工具栏）
void MV200_MainWidget::initFunctionMenuBarForm()
{
    //功能菜单工具栏（顶部工具栏）
...
//快捷按钮工具栏（左侧工具栏）
void MV200_MainWidget::initShortcutBtnBarForm()
{
    //快捷按钮工具栏（左侧工具栏）
...
//状态栏(浮子传感器、温度、试剂状态信息)
void MV200_MainWidget::initStatusBarForm()
{
    //状态栏(浮子传感器、温度、试剂状态信息)
...
//状态栏（警告、错误信息显示区）
void MV200_MainWidget::initStatusMsgBarForm()
{
    //状态栏（警告、错误信息显示区）
...
//"项目设置"下拉按钮菜单
void MV200_MainWidget::initProjectSettingMenuButton()
{
    //"项目设置"下拉按钮菜单
...
//"系统设置"下拉按钮菜单
void MV200_MainWidget::initSysSettingMenuButton()
{
    //"系统设置"下拉按钮菜单
...
//"维护"下拉按钮菜单
void MV200_MainWidget::initMaintainMenuButton()
{
...
//初始化堆叠部件
void MV200_MainWidget::initStackWidget()
{
    //堆叠部件
...
//界面风格
void MV200_MainWidget::initStyle()
{
    int width, height;
    ...
    //主界面左上角公司LOGO
    palette.setBrush(QPalette::Background, QBrush(QPixmap( QString::fromUtf8("icon/Logo.png") )));
...
//连接信号与槽
void MV200_MainWidget::initConnect()
{
    //功能菜单工具栏（顶部工具栏）
    ...
    connect(btnCleanAlertMsg, SIGNAL(clicked()), this, SLOT(slt_btnCleanAlertMsg_Clicked()));
        ...
        connect(gTcpServer, SIGNAL(sig_AlertMsg_show(int,QString)), this, SLOT(slt_AlertMsg_show(int,QString)));
    //    connect(gSerialThread2, SIGNAL(sig_AlertMsg_show(int,QString)), this, SLOT(slt_AlertMsg_show(int,QString)));

}

//初始化数据
void MV200_MainWidget::initData()
{
...
//关机动画
void MV200_MainWidget::Shutdown()
{
...
//关机
void MV200_MainWidget::slt_closeAllWindows()
{
    //退出系统
...
int MV200_MainWidget::isDataBaseHasTask()
{
    int flag = 0;
...
/*******************功能菜单工具栏（顶部工具栏）********************************************/
//槽:测试申请
void MV200_MainWidget::slt_btnTestApplication_Clicked()     {stackWidget->setCurrentWidget(ApplicationWidget::getInstance());}
//槽:工作状态
void MV200_MainWidget::slt_btnWorkStatus_Clicked()          {stackWidget->setCurrentWidget(WorkStatusWidget::getInstance());}
//槽:结果查询
void MV200_MainWidget::slt_btnResultQuery_Clicked()         {stackWidget->setCurrentWidget(ResultQueryWidget::getInstance());}
//槽:统计报表
void MV200_MainWidget::slt_btnStatisticalReport_Clicked()   {stackWidget->setCurrentWidget(ReportWidget::getInstance());}
//槽:关于
void MV200_MainWidget::slt_btnAbout_Clicked()
{
...
//"项目设置"下拉按钮菜单
//槽:常规项目
void MV200_MainWidget::slt_RoutineItem_Triggered()   {stackWidget->setCurrentWidget(RoutineProjectSettingWidget::getInstance());}
//槽:计算项目
void MV200_MainWidget::slt_ComputingItem_Triggered() {stackWidget->setCurrentWidget(CalculateProjectWidget::getInstance());}
//槽:组合项目
void MV200_MainWidget::slt_CombinateItem_Triggered() {stackWidget->setCurrentWidget(ProjectGroupWidget::getInstance());}
//槽:交叉污染
void MV200_MainWidget::slt_CrossContamination_Triggered() { }
//槽:定标设置
void MV200_MainWidget::slt_CalibrateSetting_Triggered()   { }
//槽:质控设置
void MV200_MainWidget::slt_QCSetting_Triggered()
{
...
//槽:测试顺序设置
void MV200_MainWidget::slt_TestOrderSetting_Triggered()
{
    testOrder->QueryProjectInfo();
    int x = ( QApplication::desktop()->width() - 640 ) / 2;
    int y = ( QApplication::desktop()->height() - 600 ) / 2;
    testOrder->setGeometry(x, y, 640, 600);
    testOrder->show();
}
...
//"系统设置"下拉按钮菜单
//槽:系统设置
void MV200_MainWidget::slt_SysSetting_Triggered(){sysSettingWidget->show();}
//槽:医院设置
void MV200_MainWidget::slt_HospitalSetting_Triggered() { }
//槽:LIS设置
void MV200_MainWidget::slt_LISSetting_triggered() {CommunicationSetWidget::getInstance()->show();}
//槽:时间设置
void MV200_MainWidget::slt_TimeSetting_Triggered(){}
//槽:用户管理
void MV200_MainWidget::slt_UserManager_Triggered() { }
//槽:打印设置
void MV200_MainWidget::slt_PrintSetting_Triggered() { }
//槽:条码设置
void MV200_MainWidget::slt_BarcodeSetting_Triggered() { }
//槽:英文
void MV200_MainWidget::slt_EN_Triggered() { }

//"维护"下拉按钮菜单
//槽:日常维护
void MV200_MainWidget::slt_DailyMaintain_Triggered()
{
    //    gMessageBox->setInfo(QString("提示"), QString("'日常维护'功能尚未开放!!"),
...
//槽:温度曲线
void MV200_MainWidget::slt_TemperatureCurve_Triggered()
{
    gMessageBox->setInfo(QString("提示"), QString("'温度曲线'功能尚未开放!!"),
                         QPixmap("://resource/resource/MessageBox/information.png"),  true, true);
    gMessageBox->setButtonText(Cancel, "返回");
    gMessageBox->show();
    return;
...
//槽:转移结果到历史库
void MV200_MainWidget::slt_SaveResult2History_Triggered()
{
    //    gMessageBox->setInfo(QString("提示"), QString("'转移结果'功能尚未开放!!"),
    //                         QPixmap("://resource/resource/MessageBox/information.png"),  true, true);
    //    gMessageBox->setButtonText(Cancel, "返回");
    //    gMessageBox->show();
    //    return;
    ...
    {
        gMessageBox->setInfo(QString("提示"), QString("有测试正在进行中，不能转移结果到历史库！\n请等待所有测试完成后再进行!!"),
                             QPixmap("://resource/resource/MessageBox/information.png"),  true, true);
        gMessageBox->setButtonText(Cancel, "返回");
        if(gMessageBox->exec() == MVMessageBox::Rejected)
            return;
...
//槽:进入维护（装机调试人员用）
void MV200_MainWidget::slt_Maintain_Triggered()
{
    ...
    //    gScheduler->setMV100_WorkStatus(5);
    //    gScheduler->StopIdleTimer(); //进入维护后停止-状态查询定时器
}
//槽:用户管理
void MV200_MainWidget::slt_AddNewUser_Triggered()
{
    //只有root用户才能进入“用户管理”
    ...
    int x = ( QApplication::desktop()->width() - 300 ) / 2;
    int y = ( QApplication::desktop()->height() - 600 ) / 2;
    UserManagementWidget::getInstance()->setGeometry(x, y, 100, 600);
    UserManagementWidget::getInstance()->show();
}
//槽:查看日志
void MV200_MainWidget::slt_ViewLog_Triggered()
{
    //    gMessageBox->setInfo(QString("提示"), QString("'查看日志'功能尚未开放!!"),
    //                         QPixmap("://resource/resource/MessageBox/information.png"),  true, true);
    //    gMessageBox->setButtonText(Cancel, "返回");
    //    gMessageBox->show();
    //    return;
    ...
    int y = ( QApplication::desktop()->height() - 700 ) / 2;
    ErrorMsgWidget::getInstance()->setGeometry(x, y, 800, 700);

    ErrorMsgWidget::getInstance()->show();
}
//槽:上传到LIS库
void MV200_MainWidget::slt_Upload2LIS_Triggered()
{
...
//槽:开始
void MV200_MainWidget::slt_btnStart_Clicked()
{
    int Status = gScheduler->getWorkModeFlag();
    if( Status == MV_State_Normal  || Status == MV_State_QC || Status == MV_State_CalibrateT ) //测试正在进行中，返回（开机清洗反应杯暂停后也不能）
        return;
    else if( Status == MV_State_BootReset  ) //复位失败
    {
    ...
    else if( Status == MV_State_unChecked ) //开机没有自检
    {
    ...
    else if( Status == MV_State_Stop ) //暂停
    {
...
//槽:暂停
void MV200_MainWidget::slt_btnPause_Clicked()
{
    //MV100工作状态
    if(gScheduler->get_isRunning() && gScheduler->getWorkModeFlag() != MV_State_Pause)
    {
...
//槽:停止
void MV200_MainWidget::slt_btnStop_Clicked()
{
...
//槽:锁屏
void MV200_MainWidget::slt_btnLockScreen_Clicked()   {  }
//槽:紧急关机
void MV200_MainWidget::slt_btnEmergencyShutdown_Clicked()   {  }
//槽:重启
void MV200_MainWidget::slt_btnRestart_Clicked()
{
    if(gScheduler->get_isRunning() == 1)
    {
...
//槽:关机
void MV200_MainWidget::slt_btnShutdown_Clicked()
{
...
void MV200_MainWidget::slt_timerDelay_timeout()
{
    //停止计时器
    timerDelay->stop();
...
void MV200_MainWidget::slt_setButtonState(int type)
{
    //所有测试都完成
    if(type == 0)
    {
...
//槽:温度状态（试剂槽）
void MV200_MainWidget::slt_setTemperature(double t)
{
    if(t > 40)
...
//槽:温度状态(反应盘)
void MV200_MainWidget::slt_setTemperature_Disk(double t)
{
    if(t<34)
    {
    }
...
//槽：分类4种液体的状态
void MV200_MainWidget::slt_sortOutLiquidStatus(int s)
{
...
//槽:去离子水（纯净水）
void MV200_MainWidget::slt_DeionizedWater(int s)
{
    if(s == 0) //清零表示正常
    {
        Water_run_times = 0;
    ...
    else       //去离子水少
    {
        ...
        if(Water_run_times == gParameter->get_LessWaterTimes())
        {
            ...
            {
                gScheduler->setWorkModeFlag(MV_State_Stop);
...
//槽:清洗液
void MV200_MainWidget::slt_CleaningLiquid(int s)
{
    if(s == 0) //清零表示正常
    {
...
//槽:低浓度废液
void MV200_MainWidget::slt_LowWaste(int s)
{
    if(s == 0) //清零表示正常
    {
        L_waste_run_times = 0;
...
//槽:高浓度废液
void MV200_MainWidget::slt_HighWaste(int s)
{
    if(s == 0) //清零表示正常
    {
        H_waste_run_times = 0;
...
void MV200_MainWidget::addWidgetToStack(QWidget *widget) {}
//设置当前要显示的界面
void MV200_MainWidget::showChildWidget(QWidget *widget) {}
//从stack堆上移除widget界面
void MV200_MainWidget::removeFromStack(QWidget *widget) {}


/*******************状态栏（警告、错误信息显示区）*****************************************/

//显示错误警告信息
void MV200_MainWidget::slt_AlertMsg_show(int type, QString msg)
{
...
//槽:更新状态栏上的时间
void MV200_MainWidget::slt_timer_timeout()
{
...
//槽:清除警告信息
void MV200_MainWidget::slt_btnCleanAlertMsg_Clicked() {labAlertMsg->clear(); labAlertMsgLiquid->clear();}
//槽:查看警告信息
void MV200_MainWidget::slt_btnShowAlertMsg_Clicked()// {ErrorMsgWidget::getInstance()->show();}
{
    QVector<int> ParamList;
    ParamList.append(0x04);
    ParamList.append(0x01);
    ParamList.append(0x12);
    ParamList.append(0x00);
//    ParamList.append(0x10);
//    ParamList.append(0x04);
//    ParamList.append(0x00);
//    ParamList.append(0x00);
//    ParamList.append(0x41);
//    ParamList.append(0x41);
//    ParamList.append(0x41);
//    ParamList.append(0x41);
    ...
    for(int i = 0; i < ParamList.count(); i ++)
        str += QString::number(ParamList.at(i), 16);


    qDebug() << str;
    qDebug() <<  gMV_ErrTable->getSeriousErrDescribe(0x41, str);
//    qDebug() <<  gMV_ErrTable->getNormalErrDescribe(0x45, str);
